...
from voagel.main import ERROR_COLOR, Bot
...
class AdminCommands(commands.Cog):
    ...
    module = app_commands.Group(
        name="module", description="Module commands", parent=admin
    )
        ...
        async def on_submit(self, inter: discord.Interaction) -> None:
            ...
            with redirect_stdout(stdout):
                try:
                    ...
                    embed = discord.Embed(
                        color=ERROR_COLOR, title=type(e).__name__, description=e
                    )
    ...
    async def pull(self, inter: discord.Interaction):
        ...
        out2 = await check_output(
            ["git", "log", "@{1}..", "--format=%h %an | %B"], timeout=30
        )
    ...
    async def fuck(self, inter: discord.Interaction):
        ...
        await inter.response.send_message(
            f'```{await check_output(["git", "reset", "--hard", "origin/master"])}```'
        )
