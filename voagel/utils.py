...
async def re_encode(fp: str) -> None:
    ...
    out = await check_output(
        [
            ...
            "-c:a",
            "copy",
            f"{os.path.split(fp)[0]}/2{os.path.split(fp)[1]}",
        ]
    )
...
async def check_output(
    args: typing.List[str], timeout: int = 120, raise_on_error: bool = False, cwd=None
):
    ...
    if raise_on_error and proc.returncode != 0:
        raise Exception(
            f"Shell process for `{args[0]}` exited with status `{proc.returncode}`"
        )
    return out
...
class OpenGraphParser(html.parser.HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        for attr in attrs:
            if (
                attr[0].lower() == "name"
                or attr[0].lower() == "property"
                and attr[1]
                and attr[1].startswith("og:")
            ):
                k = attr[1]
