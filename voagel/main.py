...
class Bot(commands.Bot):
    ...
    async def on_ready(self) -> None:
        self.session = aiohttp.ClientSession(
            headers={
                "User-Agent": "Voagel (Discord Bot) https://github.com/Vocaned/Voagel"
            }
        )
    ...
    async def setup_hook(self) -> None:
        ...
        for extension in [
            ...
            .replace("/", ".")
            .replace("\\", ".")
            .split("voagel.extensions.")[-1]
            for f in Path(__file__).parent.rglob("extensions/**/*.py")
        ]:
            try:
    ...
    def get_api_key(self, key: str) -> str:
        ...
        if not val:
            raise commands.DisabledCommand(
                "API key for command missing. Ping the bot owner for help."
            )
        return val
    ...
    def load_config(self) -> None:
        ...
        if not Path("config", "bot.toml").exists():
            logging.critical(
                "Bot config could not be found. Copy bot.toml to the config folder and edit it to continue."
            )
            sys.exit(1)
...
def main() -> None:
    ...
    bot.activity = discord.Activity(
        name=bot.config["bot"]["activity"], type=activitytype
    )
